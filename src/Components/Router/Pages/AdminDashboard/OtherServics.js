...
const OtherServices = () => {
    ...
    const handleAddServices = event => {
        ...
        const name = event.target.otherServices.value;
        const servicesCategory = 'Other services';
    ...
    const handleDlt = (_id) =>{
        ...
        if(agree){
            console.log(_id)
           fetch(`https://origin-hospital-server.vercel.app/delete-other-service/${_id}`,{
            method: 'DELETE'})
            .then(res => res.json())
            .then(data => {
    ...
    return (
        ...
        {isShown2 ?
            <>
                    ...
                    <form onSubmit={handleAddServices}>
                        <input className="input input-bordered w-full max-w-3xl  
                            ...
                            mx-auto p-5"   type={"text"} name={'otherServices'} placeholder={"Edit outdoor services"}>
                        </input>
                       
                        <br />
                        <button className='btn  btn-error  btn-sm m-5 mt-1 ml-1' type="submit">Add service</button>
            ...
            :
            <>
                <button className='btn  btn-error  m-5 ' onClick={handleClick2}>Add service</button>
            </>
        }
    </div>
    );
